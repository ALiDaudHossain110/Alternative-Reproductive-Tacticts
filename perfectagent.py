...
class Agent(pg.sprite.Sprite):
    ...
    def act(self,agents,food_group):
        ...
        if self.state == 'mating as male' and self.nearby_agents:
            self.find_mate_as_male(agents)
        else:
            self.genome.fitness-=10000
            self.state=='wandering'

        if self.state == 'mating as female'and self.nearby_agents:
            self.find_mate_as_female(agents)
        else:
            self.genome.fitness-=10000
    ...
    def find_mate_as_male(self,agents):
        self.gender="m"
    ...
    def find_mate_as_female(self,agents):
        ...
        if (self.gender == "m" and other.gender == "f") or (self.gender == "f" and other.gender == "m"):
            ...
            if male.energy_level > 3 and female.energy_level > 3:
                if male.age > 7 and female.age > 7 and female.can_reproduce:
                    ...
                    if self.gender=="m":                        
                        self.no_of_matings_as_male+=1
                    
                    if self.gender=="f":                        
                        self.no_of_matings_as_female+=1
                    

                    for _ in range(num_children):
                        c.genomeid+= 1
                        #child_genome = neat.DefaultGenome(c.genomeid)  # Create a new genome for the child
                        male.genome.fitness += 2
                        female.genome.fitness += 15
    ...
    def search_for_food(self, agents, food_group):
        ...
        l = self.left_wheel_speed
        r = self.right_wheel_speed
        ...
        if choice == "s":
            ...
            l *= 1.25
            r *= 1.25
            l = -l  # Reverse one side for turning
        elif choice == "r":
            l *= 1.25
            r *= 1.25
            r = -r  # Reverse the other side for turning

        # Clamp the wheel speeds to prevent them from growing too large
        # l = min(max(l, -4.5), 4.5)
        # r = min(max(r, -4.5), 4.5)
        l=min(l,4.5)
        r=min(r,4.5)
        # Update the wheel speeds
        self.left_wheel_speed = round(l, 2)
        self.right_wheel_speed = round(r, 2)

        # Debugging prints to ensure clamping works
        if self.left_wheel_speed > 4.5 or self.right_wheel_speed > 4.5:
